...
import { RPC_URL, REGISTRY_CONTRACT_ID, DISTRIBUTOR_CONTRACT_ID } from './config'
...
/**
 * Fetches events emitted since the last poll for both the registry and
 * distributor contracts, powering the live payout feed.
 */
export async function fetchNewEvents() {
  ...
  const contractIds = [REGISTRY_CONTRACT_ID, DISTRIBUTOR_CONTRACT_ID].filter(Boolean)
